...
Из описаных ранее условий удалим из доверенных _Manager_-а. Соотвественно каналы связи с ним также становятся не доверенными. И _Storage_, и _Manager_, выступая посредниками взаимодействия _Verifier_-а и _Updater_-а, имеют возможность реализовать [MITM-атаку](https://encyclopedia.kaspersky.ru/glossary/man-in-the-middle-attack/).

Таким образом, необходимо обеспечить достоверность того факта, что файл-обновления, устанавливаемый _Updater_-ом в действительности является тем же самым, который проверялся _Verifier_-ом.

На мой взгляд, единственным вариантом решения задачи явлвяется применеие шифрования канала передачи данных, например, части данных - цифровой подписи файла-обновления. 

В рамках учебного примера представляется возможным обойтись без асимметричного шифрования, как это применяется при технологии электронной цифровой подписи. Предлагаю обойтись следующим.

__Предположение безопасности__:

Сведения о секретном ключе шифрования доступны исключительно _Updater_-у и _Verifier_-у. 

Речь именно о ключе, а не алгоритме.

### Подпись файла-обновления

В результате проверки файла-обновления _Verifier_ подписывает его по алгоритму хеширования с "солью", где "соль" - это и есть "секретный ключ" из предположения безопасности выше:

```python
SECRET_SALT = b'S0M3$3CR3TS#1T' # "секретный ключ", "соль"

def set_up_data_digest(blob_data) -> str:  # функция генерации подписи к данным - хеширования с "солью"
    if not blob_data:
        return None
    payload_digest = sha256()
    update_payload = base64.b64decode(blob_data)
...
Далее _Updater_, получая от _Storage_ файл-обновления, сверяет полученную предположительно от _Verifier_-а подпись `details['verified_digest']` со значением, вычисленным самостоятельно (он в любом случае должен реализовывать какую-то проверку):
...
попираясь на тот же алгоритм, что и _Verifier_, - хеширование с "солью":

```python
SECRET_SALT = b'S0M3$3CR3TS#1T' # "секретный ключ", "соль"

def validate_verifier_digest(blob_data, verifier_digest) -> bool: # функция порверки подписи
    if not blob_data:
        return False
...
В случае компрометации файла-обновления со стороны _Storage_ или _Manager_-а им необходимо подделать в потоке данных и цифровую подпись _Verifier_-а (`details['verified_digest']`). Но в силу нашего предположения безопасности _Storage_ и _Manager_, даже имея сведения об используемом алгоритме шифрования (в частности по общему виду хеш-свертки), не владеют данными о подмешиваемой в алгоритме "соли" (`SECRET_SALT`).

__Заметка__: повысить устойчивость "соли" к компрометации возможно путем ее динамического генерирования с указанием времени жизни. Такой подход усложнит реализацию для демонстрации и разбора примера с недоверенным _Manager_-ом. Да и видится, что асимметричное шифрование для ЭЦП файла-обновления реализовать будет проще("дешевле" и "быстрее"), чем приведенное "динамическое подсаливание". При этом сохранность секретных ключей в случае асимметричного шифрования также будет необходимо безоговорочно обеспечивать, что укладывается в предположения безопасности.

Итак. Результат работы в случае компрометации файла-обновления будет выглядеть так:
...
как видим - "verifier digest is not valid".
